...
def base_merchant_title(parent_title):
    """
    Turn short Tilda catalog names into clear Shopping titles.
    We only add wording that is objectively true for the product category.
    Unknown/specific product names are preserved.
    """
    original = clean(parent_title)
    low = original.lower()

    # Remove a size already baked into the parent title for rollers,
    # because the selected edition size is appended separately below.
    ...
    if low.startswith("черпак") or low.startswith("ковш"):
        return "Черпак для бани Banya Line"

    if low.startswith("халат"):
        return "Халат для бани Banya Line"

    if low.startswith("костюм"):
        return "Костюм для бани Banya Line"

    if low.startswith("вешалка"):
        return f"{original} для бани Banya Line" if "бан" not in low else f"{original} Banya Line"

    if low.startswith("полка"):
        return f"{original} для бани Banya Line" if "бан" not in low else f"{original} Banya Line"
    ...
    # Preserve specific/less predictable names instead of inventing semantics.
...
def edition_title_attributes(edition):
    ...
    for key, raw in edition.items():
        ...
        value = normalize_dimension(label or raw_value)
        ...
        if "цвет" in key_l:
            color = value.lower().replace("черный", "чёрный").replace("черный", "чёрный")
            continue
