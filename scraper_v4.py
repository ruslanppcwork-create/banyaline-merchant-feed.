...
# Images used globally by the site/template rather than by a specific product.
BAD_IMAGE_FILENAMES = {
    "frame_755829.jpg",
}
...
def is_image_asset(url: str) -> bool:
    ...
    if any(x in lower for x in ("logo", "favicon", "/icons/", ".svg")):
        return False
    if image_filename(url) in BAD_IMAGE_FILENAMES:
        return False
    return bool(
        re.search(r"\.(?:jpe?g|png|webp)(?:\?.*)?$", lower)
        or "tildacdn" in lower
    )


def collect_image_candidates(soup: BeautifulSoup, html: str, page_url: str) -> list[str]:
    """Collect image URLs in approximate document order."""
    candidates = []

    def add(value):
        if not value:
            return
        # data-product-img can contain JSON or multiple values
        image = normalize_image_url(value, page_url)
        if is_image_asset(image) and image not in candidates:
            candidates.append(image)

    # HTML attributes first: these preserve DOM order.
    attrs = (
        "data-product-img",
        "data-product-image",
        "data-product-photo",
    ...
    # Raw source fallback catches images embedded in JS objects.
    raw = html.replace("\\/", "/")
    patterns = [
        r'https?://static\.tildacdn\.(?:com|biz)/[^"\'<>\s\\]+?\.(?:jpe?g|png|webp)(?:\?[^"\'<>\s\\]*)?',
        r'//static\.tildacdn\.(?:com|biz)/[^"\'<>\s\\]+?\.(?:jpe?g|png|webp)(?:\?[^"\'<>\s\\]*)?',
    ]
...
def collect_global_images() -> set[str]:
    """
    Images appearing on non-product reference pages are treated as template/site
    assets and should not be used as Merchant product photos.
    """
    global_images = set()
    for url in GLOBAL_REFERENCE_URLS:
        try:
            response = get(url)
            soup = BeautifulSoup(response.text, "html.parser")
            global_images.update(
                collect_image_candidates(soup, response.text, url)
            )
        except Exception as exc:
            print(f"[WARN] Cannot build global image blacklist from {url}: {exc}")
    print(f"Global/template image blacklist: {len(global_images)} images")
...
def images_from_matching_product_block(
    soup: BeautifulSoup,
    page_url: str,
    title: str,
    global_images: set[str],
) -> list[str]:
    """
    Prefer images from a product block whose text/title resembles the current
    product. This avoids picking recommendation cards from the same page.
    """
    title_norm = re.sub(r"\s+", " ", title.lower()).strip()
    title_words = [
        w for w in re.findall(r"[a-zа-яё0-9]+", title_norm, flags=re.I)
        if len(w) >= 4
    ]
    ...
    for selector in selectors:
        for block in soup.select(selector):
            ...
            score = sum(1 for w in title_words if w in block_text)

            # Link to current URL/slug is a very strong signal.
            slug = slug_from_url(page_url).lower()
            hrefs = " ".join(
                clean_text(a.get("href")).lower()
                for a in block.find_all("a", href=True)
            )
            if slug and slug in hrefs:
                score += 10
            ...
            imgs = collect_image_candidates(block, block_html, page_url)
            imgs = [
                x for x in imgs
                if x not in global_images and image_filename(x) not in BAD_IMAGE_FILENAMES
            ]
            if imgs:
...
def extract_image(
    ...
    # 1. Structured-data image only if it is not a known template/global asset.
    structured = normalize_image_url(first_image_from_jsonld(product), page_url)
    if (
        is_image_asset(structured)
        and structured not in GLOBAL_IMAGES
        and image_filename(structured) not in BAD_IMAGE_FILENAMES
    ):
        return structured

    # 2. Best heuristic: image from a product block matching the current title/url.
    matched = images_from_matching_product_block(
        soup, page_url, title, GLOBAL_IMAGES
    )
    if matched:
        return matched[0]

    # 3. Page-specific images: subtract site-wide/template images.
    candidates = collect_image_candidates(soup, html, page_url)
    page_specific = [
        x for x in candidates
        if x not in GLOBAL_IMAGES
        and image_filename(x) not in BAD_IMAGE_FILENAMES
    ]

    if page_specific:
        return page_specific[0]

    # Do NOT fall back to og:image: on this Tilda site it is a generic image
    # and would corrupt the Merchant feed.
    return ""
...
def parse_product(url: str) -> dict | None:
    ...
    image = extract_image(product, soup, html, url, title)
...
def main():
    ...
    print("Building site-wide image blacklist...")
    GLOBAL_IMAGES = collect_global_images()
    ...
    if len(products) >= 10 and most_common_count / len(products) > 0.15:
        raise RuntimeError(
    ...
    bad_in_feed = [
        p for p in products
        if image_filename(p["image_link"]) in BAD_IMAGE_FILENAMES
    ]
    if bad_in_feed:
        raise RuntimeError(
            f"Known template image leaked into feed for {len(bad_in_feed)} products."
        )

    # Require reasonable image diversity. Variants may legitimately share an
    # image, so this is intentionally not a 1:1 requirement.
    if len(products) >= 20 and len(image_counts) < max(8, len(products) // 5):
        raise RuntimeError(
            "Too few unique product images: "
            f"{len(image_counts)} unique images for {len(products)} products."
        )
