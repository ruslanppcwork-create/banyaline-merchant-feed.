...
def normalize_dimension(value):
    """Make dimensions consistent: 23х60 / 23x60 -> 23×60."""
    value = clean(value)
    value = re.sub(r"(?<=\d)\s*[xхХX]\s*(?=\d)", "×", value)
    return value


def base_merchant_title(parent_title):
    """
    Build a clean Shopping title base from short Tilda product names.
    Unknown/specific names are preserved rather than rewritten aggressively.
    """
    original = clean(parent_title)
    low = original.lower()

    if "вентиляция" in low and "второе дыхание" in low:
        return "Вентиляция для бани «Второе дыхание» Banya Line"

    if "инженерная вентиляция" in low and "басту" in low:
        return "Инженерная вентиляция Басту для парной Banya Line"

    if "коллекция банных черпаков" in low or low.startswith("черпак") or low.startswith("ковш"):
        return "Черпак для бани Banya Line"

    if low.startswith("валик"):
        return "Валик для бани Banya Line"

    if low == "матрас":
        return "Матрас для бани Banya Line"

    if "матрас" in low and "второе дыхание" in low:
        return "Матрас для бани «Второе дыхание» Banya Line"

    if "проливной матрас" in low and "второе дыхание" in low:
        return "Проливной матрас для бани «Второе дыхание» Banya Line"

    if low.startswith("проливной матрас"):
        return "Проливной матрас для бани Banya Line"

    if low == "подушка":
        return "Подушка для бани Banya Line"

    if "подушка" in low and "второе дыхание" in low:
        return "Подушка для бани «Второе дыхание» Banya Line"

    if low == "килт":
        return "Килт для бани Banya Line"

    if low == "парео":
        return "Парео для бани Banya Line"

    if low == "веер":
        return "Веер для бани Banya Line"

    if low.startswith("полотен"):
        return "Полотенце для бани Banya Line"

    if low.startswith("простын"):
        return "Простыня для бани Banya Line"

    if "перенос" in low and "дров" in low:
        return "Переноска для дров Banya Line"

    if low == "запарник":
        return "Запарник для веников Banya Line"

    if low == "запарка":
        return "Запарка для бани Banya Line"

    if "фито-полка" in low:
        return f"{original} для бани Banya Line"

    if "вешалка" in low or "полка" in low:
        if "для бани" in low:
            return f"{original} Banya Line"
        return f"{original} для бани Banya Line"

    if "панно" in low:
        return f"{original} для бани Banya Line" if "для бани" not in low else f"{original} Banya Line"

    if low.startswith("комплект. второе дыхание"):
        return "Комплект для бани «Второе дыхание» Banya Line"

    if "полный комплект" in low:
        return "Полный комплект для бани Banya Line"

    if "комплект" in low or "набор" in low:
        if "бан" in low or "парен" in low:
            return f"{original} Banya Line"
        return f"{original} для бани Banya Line"

    if "бокс из трав и веников" in low:
        return "Бокс из трав и веников для бани Banya Line"

    if "banya line" in low:
        return original

    return f"{original} Banya Line"

def normalize_option_value(value):
    """Fix raw Tilda option labels that are technically valid but ugly in ads."""
    value = normalize_dimension(clean(value))
    key = value.lower().replace("ё", "е").strip()

    replacements = {
        "черный": "чёрный",
        "черныйметалл": "чёрный металл",
        "нежныйперсик": "нежный персик",
        "темы-синий": "тёмно-синий",
...
def edition_title_attributes(edition):
    """
    Return variant attributes in a readable order.
    Example:
      Размер=60×200 см -> 60×200 см
      Цвет=Черный:/catalog/... -> цвет чёрный
      Длина черпака=560 мм -> длина 560 мм
    """
    size = ""
    color = ""
    extras = []
    ...
    for key, raw in edition.items():
        ...
        label, _path = catalog_path_from_option(raw_value)
        value = normalize_option_value(label or raw_value)
        ...
        if "длина" in key_l:
            extras.append(f"длина {value}")
            continue

        if "форма ручки" in key_l:
            extras.append(f"ручка {value.lower()}")
            continue

        if "формат" in key_l:
            extras.append(f"формат {value.lower()}")
            continue

        # For uncommon options keep both meaning and selected value.
        if value:
            extras.append(f"{clean(key).lower()} {value}")

    attrs = []
    if size:
        attrs.append(size)

    # Some Tilda products use the "Цвет" option for material/finish values.
    # Avoid phrases such as "цвет металл" or "цвет чёрный металл".
    if color:
        if color in {"металл", "чёрный металл", "сталь"}:
            attrs.append(color)
        else:
...
def build_merchant_title(parent_title, edition=None):
    """
    Final Google Merchant title.
    Keeps the main product query near the beginning and variant details after it.
    """
    base = base_merchant_title(parent_title)
    attrs = edition_title_attributes(edition or {})

    # Remove attributes already clearly contained in the base title.
    base_low = normalize_dimension(base).lower()
    clean_attrs = []
    for attr in attrs:
        attr_low = normalize_dimension(attr).lower()
        # Exact duplicate/contained dimensions or phrases shouldn't repeat.
        if attr_low and attr_low not in base_low and attr not in clean_attrs:
            clean_attrs.append(attr)

    title = base
    if clean_attrs:
        title += ", " + ", ".join(clean_attrs)

    return title[:150].rstrip(" ,")
    ...
    if by_link:
        for link, data in by_link.items():
            ...
            if not price or not image:
                continue
            merchant_title = build_merchant_title(parent_title, edition)
            offers.append({
        ...
        if parent_url and parent_price and image:
            offers.append({
                ...
                "item_group_id": parent_uid,
                "title": build_merchant_title(parent_title),
                "description": parent_descr[:5000] if parent_descr else generic_description(parent_title, ptype),
                "link": parent_url,
                "image_link": image,
...
def main():
    ...
    for p in products:
        ...
        sample_title = generated[0]["title"] if generated else "-"
        print(f"{clean(p.get('title'))}: {len(generated)} offer(s) | {sample_title}")
